...
func _physics_process(delta):
	if Input.is_action_just_pressed("ui_accept"): 
		if esta_subindo:
			...
			tentar_subir() # Se está no chão, a ação é tentar subir
	if esta_subindo:
		return
	...
	if raycast_interacao.is_colliding():
		...
		if obj.has_method("empurrar") and input_direction != Vector2.ZERO:
			...
			print("Forçando... Tempo: ", cronometro_empurrao) # Isso tem que aparecer no console aumentando
			if cronometro_empurrao >= tempo_para_empurrar:
				print("SUCESSO: Empurrando agora!")
				# Trava em 4 direções
